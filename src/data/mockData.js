// src/data/mockData.js

// export default 讓我們可以在其他檔案中引入這個陣列
export default [
  {
    id: 1,
    imageUrl: 'https://picsum.photos/id/10/400/533', // 使用 picsum.photos 來產生假圖片
    userAvatar: 'https://i.pravatar.cc/150?u=a042581f4e29026704d',
    userName: 'Aki'
  },
  {
    id: 2,
    imageUrl: 'https://picsum.photos/id/20/400/533',
    userAvatar: 'https://i.pravatar.cc/150?u=a042581f4e29026704e',
    userName: 'Yuki'
  },
  {
    id: 3,
    imageUrl: 'https://picsum.photos/id/30/400/533',
    userAvatar: 'https://i.pravatar.cc/150?u=a042581f4e29026704f',
    userName: 'Mika'
  },
  {
    id: 4,
    imageUrl: 'https://picsum.photos/id/40/400/533',
    userAvatar: 'https://i.pravatar.cc/150?u=a042581f4e29026704a',
    userName: 'Haru'
  },
  {
    id: 5,
    imageUrl: 'https://picsum.photos/id/50/400/533',
    userAvatar: 'https://i.pravatar.cc/150?u=a042581f4e29026704b',
    userName: 'Sora'
  },
  {
    id: 6,
    imageUrl: 'https://picsum.photos/id/60/400/533',
    userAvatar: 'https://i.pravatar.cc/150?u=a042581f4e29026704c',
    userName: 'Rin'
  }
]